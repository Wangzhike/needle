...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node_i in reverse_topo_order:
      ...
      assert(node_i in node_to_output_grads_list)
      adjoint_ij_lst = node_to_output_grads_list[node_i]
      assert(len(adjoint_ij_lst) != 0)
      adjoint_i = adjoint_ij_lst[0]
      for j in range(1, len(adjoint_ij_lst)):
        adjoint_i += adjoint_ij_lst[j]
      node_i.grad = adjoint_i
      assert(node_i.grad.dtype == array_api.float32)

      
      ### Compute partital adjoints of node_i
      if not node_i.op:
        continue
      adjoint_ki_tuple = node_i.op.gradient(node_i.grad, node_i)
      assert(len(node_i.inputs) == len(adjoint_ki_tuple))
      ### "Propagation" partial adjoint to its inputs
      for k in range(len(node_i.inputs)):
        node_k = node_i.inputs[k]
        adjoint_ki = adjoint_ki_tuple[k]
        assert(adjoint_ki.dtype == array_api.float32)
        if node_k not in node_to_output_grads_list:
          node_to_output_grads_list[node_k] = list()
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    for prev in node.inputs:
      if prev not in visited:
        visited.add(prev)
        topo_sort_dfs(prev, visited, topo_order)
